...
describe('interpolateSqlVariables', () => {
	...
	it('joins multi-value strings with comma for IN clause', () => {
		...
		expect(interpolateSqlVariables('WHERE name IN ($names)', vars)).toBe(
	...
	it('handles multiple variables in one query', () => {
		...
		expect(interpolateSqlVariables('WHERE dataset = $dataset LIMIT $limit', vars)).toBe(
			"WHERE dataset = 'workers' LIMIT 10",
		);
	});
});
