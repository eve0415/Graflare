...
describe('render methods', () => {
  it('rate wraps bare selector with range', () => {
    ...
    const result = must(entry).render('http_requests_total', [{ kind: 'range', value: '5m' }]);
  ...
  it('rate uses subquery form when inner is already wrapped', () => {
    const entry = catalogByName.get('rate');
    expect(entry).toBeDefined();
    const result = must(entry).render('sum by (job)(http_requests_total)', [{ kind: 'range', value: '5m' }]);
  ...
  it('irate renders with range', () => {
    ...
    expect(entry).toBeDefined();
    expect(must(entry).render('metric', [{ kind: 'range', value: '1m' }])).toBe('irate(metric[1m])');
  });

  it('sum renders with grouping', () => {
    const entry = catalogByName.get('sum');
    expect(entry).toBeDefined();
    const result = must(entry).render('metric', [{ kind: 'grouping', mode: 'by', labels: ['job'] }]);
  ...
  it('sum renders without grouping', () => {
    const entry = catalogByName.get('sum');
    expect(entry).toBeDefined();
    expect(must(entry).render('metric', [])).toBe('sum(metric)');
  });

  it('topk renders scalar-first', () => {
    const entry = catalogByName.get('topk');
    expect(entry).toBeDefined();
    const result = must(entry).render('metric', [{ kind: 'scalar', value: '5' }]);
  ...
  it('bottomk renders scalar-first', () => {
    ...
    expect(entry).toBeDefined();
    const result = must(entry).render('metric', [{ kind: 'scalar', value: '3' }]);
  ...
  it('histogram_quantile renders scalar-first', () => {
    ...
    expect(entry).toBeDefined();
    const result = must(entry).render('metric', [{ kind: 'scalar', value: '0.95' }]);
  ...
  it('quantile aggregation renders scalar-first with grouping', () => {
    ...
    expect(entry).toBeDefined();
    const result = must(entry).render('metric', [
  ...
  it('round renders without precision when default', () => {
    ...
    expect(entry).toBeDefined();
    expect(must(entry).render('metric', [])).toBe('round(metric)');
    expect(must(entry).render('metric', [{ kind: 'scalar', value: '1' }])).toBe('round(metric)');
  });

  it('round renders with custom precision', () => {
    const entry = catalogByName.get('round');
    expect(entry).toBeDefined();
    expect(must(entry).render('metric', [{ kind: 'scalar', value: '0.1' }])).toBe('round(metric, 0.1)');
  });

  it('abs renders as simple wrapper', () => {
    const entry = catalogByName.get('abs');
    expect(entry).toBeDefined();
    expect(must(entry).render('metric', [])).toBe('abs(metric)');
  });

  it('aggregation with without mode', () => {
    const entry = catalogByName.get('avg');
    expect(entry).toBeDefined();
    const result = must(entry).render('metric', [{ kind: 'grouping', mode: 'without', labels: ['instance'] }]);
