...
const base64UrlToString = (segment: string): string =>
  atob(segment.replaceAll("-", "+").replaceAll("_", "/"))

const decodeJwtPayload = (
  token: string,
): { header: JwtHeader; payload: AccessJwtPayload } => {
  ...
  const payload = parseAccessJwtPayload(
    JSON.parse(base64UrlToString(payloadPart)),
  )
...
const verifyJwt = async (
  ...
  const signature = Uint8Array.from(
    base64UrlToString(signaturePart),
    (c) => c.codePointAt(0) ?? 0,
  )
...
export const accessMiddleware =
  (): MiddlewareHandler<AppEnv> => async (c: Context<AppEnv>, next) => {
