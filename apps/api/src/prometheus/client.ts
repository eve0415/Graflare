...
export class PrometheusClient {
  ...
  async labelValues(label: string, match?: string[]): Promise<PrometheusResponse> {
    const params = new URLSearchParams();
    if (match) {
      for (const m of match) params.append('match[]', m);
    }
  ...
  private async post(path: string, params: Record<string, string>): Promise<PrometheusResponse> {
    const url = new URL(this.baseUrl);
    url.pathname = url.pathname.replace(/\/$/, '') + path;
  ...
  private errorResponse(error: unknown): PrometheusResponse {
    ...
    return {
      ...
      error: isTimeout ? `Query timed out after ${this.timeoutMs / 1000}s` : message,
    };
  }
}
