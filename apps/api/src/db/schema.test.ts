...
describe('d1 schema', () => {
  ...
  it('updates a datasource', async () => {
    ...
    await db.insert(organizations).values(testOrg);
    await db.insert(datasources).values(testDatasource);
    await db.update(datasources).set({ name: 'Updated Name' }).where(eq(datasources.id, testDatasource.id));
    const rows = await db.select().from(datasources).where(eq(datasources.id, testDatasource.id));
    const [row] = rows;
  ...
  it('stores and retrieves encrypted credentials', async () => {
    const { db } = setup();
    await db.insert(organizations).values(testOrg);
    await db.insert(datasources).values({ ...testDatasource, credentials: 'encrypted-blob' });
    const rows = await db.select().from(datasources);
    const [row] = rows;
