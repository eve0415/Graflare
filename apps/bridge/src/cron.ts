...
import {
  getEnabledDatasets,
  getIntrospectedConfigs,
  runDiscovery,
  runSchemaDiscovery,
  shouldRunSchemaDiscovery,
  shouldRunSettingsDiscovery,
} from './discovery';
...
export const computeBackoff = (attempts: number): number => Math.min(BACKOFF_BASE_S * 2 ** attempts, BACKOFF_CAP_S);
...
const clearDatasetStatus = async (db: ReturnType<typeof drizzle>, dataset: string, scope: string, scopeId: string): Promise<void> => {
  await db.delete(datasetStatus).where(and(eq(datasetStatus.dataset, dataset), eq(datasetStatus.scope, scope), eq(datasetStatus.scopeId, scopeId)));
};
...
const stripDeniedFieldFromCache = async (db: ReturnType<typeof drizzle>, nodeName: string, scope: string, fieldName: string): Promise<boolean> => {
  const rows = await db
    .select()
    .from(schemaCache)
  ...
  const dimFields = Array.isArray(rawDims) ? rawDims.filter((f): f is string => typeof f === 'string') : [];
...
const processCollectorResult = async (
  ...
  try {
    ...
    console.error(
      JSON.stringify({
        level: 'error',
        event: 'sync_state_update_failed',
...
const extractScopeData = (response: GraphQLResponse<Record<string, unknown>>, scopeNode: string): Record<string, unknown> | null => {
  const { data } = response;
...
const processGraphQLBatch = async (
  ...
  const response: GraphQLResponse<Record<string, unknown>> = await cfGraphQL(env.CF_API_TOKEN, query, variables, {
  ...
  if (response.data === null) {
    ...
    const errClass =
      response.errors !== undefined && response.errors.length > 0 ? classifyError(response.errors[0] ?? { message: '' }) : ('unknown' satisfies ErrorClass);
