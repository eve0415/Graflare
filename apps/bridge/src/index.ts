...
const isSqlRequestBody = (body: unknown): body is SqlRequestBody => typeof body === 'object' && body !== null && 'sql' in body && typeof body.sql === 'string';
...
app.post('/sql', async c => {
  ...
  try {
    ...
    const columnNames = result.results.length > 0 ? Object.keys(result.results[0] ?? {}) : [];
