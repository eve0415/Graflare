...
const countConfigFields = (c: { time: { field?: string }; dimKeys: readonly string[]; resourceDimension: string; metrics: readonly unknown[] } | undefined): number => {
...
const getMetricSources = (c: ReturnType<typeof schemaToConfig>): string[] =>
	c?.metrics.map((m) => m.source) ?? [];
