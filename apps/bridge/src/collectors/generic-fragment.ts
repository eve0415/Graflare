...
const collectDimensionFields = (config: DatasetConfig): string[] => {
	...
	if (
		config.resourceDimension !== '_all'
		&& config.resourceDimension !== '_scopeId'
		&& !fields.has(config.resourceDimension)
	) {
...
export const buildAlias = (config: DatasetConfig): string =>
	config.datasetName.replaceAll(/-([a-z])/g, (_, ch: string) => ch.toUpperCase());

export const buildTimeVarType = (config: DatasetConfig): 'Time' | 'Date' =>
	config.filter.kind === 'time' ? 'Time' : 'Date';
