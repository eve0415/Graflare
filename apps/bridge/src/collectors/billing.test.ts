...
describe('billing collector', () => {
	...
	it('throws on non-200 response', async () => {
		...
		await expect(
			billingCollector.run(testEnv, '2026-06-05T00:00:00Z', '2026-06-05T12:00:00Z'),
		).rejects.toThrow('Billing API returned 403');
	});
	...
	it('skips items with invalid shape', async () => {
		const mockResponse = {
			result: [
				{ serviceName: 'workers', billingCurrency: 'USD', consumedQuantity: 100, contractedCost: 1 },
				{ invalid: true },
			],
		};
