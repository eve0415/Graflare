...
const isTimeDimension = (field: string): boolean => field.startsWith('datetime') || field === 'date';

const isResourceLike = (field: string): boolean => field.endsWith('Name') || field.endsWith('Id') || field.endsWith('Tag');

const camelToKebab = (s: string): string => s.replaceAll(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
...
export const toDatasetName = (nodeName: string): string => camelToKebab(stripSuffix(nodeName));

const pickTimeDimension = (dimensionFields: readonly string[], override: DatasetOverride | undefined): string | undefined => {
  if (override?.preferredTimeDim !== undefined) {
    return dimensionFields.includes(override.preferredTimeDim) ? override.preferredTimeDim : undefined;
  }
...
const pickResourceDimension = (dimensionFields: readonly string[], timeDim: string | undefined, override: DatasetOverride | undefined): string => {
  if (override?.resourceDimension !== undefined) return override.resourceDimension;
  return dimensionFields.find(f => !isTimeDimension(f) && isResourceLike(f) && f !== timeDim) ?? '_all';
};

const buildMetrics = (fields: IntrospectedFields, budget: number): DatasetConfig['metrics'] => {
