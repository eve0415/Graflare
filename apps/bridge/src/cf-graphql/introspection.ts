...
export const discoverScopeDatasets = async (token: string, scope: 'account' | 'zone'): Promise<DatasetNode[]> => {
  ...
  for (const field of fields) {
    ...
    const { args } = field;
    const hasFilterArg = Array.isArray(args) && args.some((a: unknown) => isRecord(a) && a['name'] === 'filter');
...
export const introspectDatasetFields = async (token: string, typeName: string): Promise<IntrospectedFields> => {
  ...
  const hasCount = Array.isArray(rootFields) && rootFields.some((f: unknown) => isRecord(f) && f['name'] === 'count');
