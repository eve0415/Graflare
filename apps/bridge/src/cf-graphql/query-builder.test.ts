...
describe('buildBatchedQuery', () => {
	...
	it('builds account-scoped query with Time variables', () => {
		const query = buildBatchedQuery('account', [
			fakeCollector({ fragment: 'workers: workersData { count }', timeVarType: 'Time' }),
		]);
	...
	it('builds zone-scoped query', () => {
		const query = buildBatchedQuery('zone', [
			fakeCollector({ scope: 'zone', fragment: 'http: httpData { count }', timeVarType: 'Time' }),
		]);
	...
	it('includes Date variables when needed', () => {
		const query = buildBatchedQuery('account', [
			fakeCollector({ fragment: 'd1: d1Data { count }', timeVarType: 'Date' }),
		]);
