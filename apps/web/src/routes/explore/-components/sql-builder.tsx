...
const ColumnToggle = ({ col, isSelected, columns, onUpdate }: ColumnToggleProps) => {
  const handleClick = useCallback(() => {
    const next = isSelected ? columns.filter(c => c !== col) : [...columns, col];
    onUpdate(next);
  }, [col, isSelected, columns, onUpdate]);

  return (
    <Button variant={isSelected ? 'secondary' : 'ghost'} size='xs' onClick={handleClick}>
      {col}
    </Button>
  );
};
...
const GroupByToggle = ({ col, isSelected, groupBy, onUpdate }: GroupByToggleProps) => {
  const handleClick = useCallback(() => {
    const next = isSelected ? groupBy.filter(c => c !== col) : [...groupBy, col];
    onUpdate(next);
...
const WhereRowWrapper = ({ index, columns, clause, onUpdate, onRemove }: WhereRowWrapperProps) => {
  ...
  return <SqlBuilderWhereRow columns={columns} clause={clause} onChange={handleChange} onRemove={handleRemove} />;
};
...
const OrderByRow = ({ index, clause, columnNames, onUpdate, onRemove }: OrderByRowProps) => {
  ...
  return (
    <div className='flex items-center gap-1.5'>
      ...
      <Select value={clause.direction} onValueChange={handleDirectionChange} items={ORDER_DIRECTION_OPTIONS}>
...
export const SqlBuilder = ({ datasourceId, state, onStateChange }: SqlBuilderProps) => {
  ...
  const removeWhere = useCallback(
    (index: number) => {
      updateField(
        'where',
        state.where.filter((_, i) => i !== index),
      );
    },
    [state.where, updateField],
  );
  ...
  const removeOrderBy = useCallback(
    (index: number) => {
      updateField(
        'orderBy',
        state.orderBy.filter((_, i) => i !== index),
      );
    },
    [state.orderBy, updateField],
  );
  ...
  return (
    <div className='flex flex-col gap-3'>
      ...
      <div className='flex items-center gap-2'>
        ...
        <Select value={state.table} onValueChange={handleTableChange} items={tableItems}>
      ...
      <div className='flex items-start gap-2'>
        ...
        <div className='flex flex-wrap gap-1'>
          <Button variant={state.columns.length === 0 ? 'secondary' : 'ghost'} size='xs' onClick={clearColumns}>
            All (*)
          </Button>
          {columnNames.map(col => (
            <ColumnToggle key={col} col={col} isSelected={state.columns.includes(col)} columns={state.columns} onUpdate={updateColumns} />
          ))}
        </div>
      </div>
      ...
      <div className='flex items-start gap-2'>
        ...
        <div className='flex flex-col gap-1.5'>
          {state.where.map((clause, i) => (
            <WhereRowWrapper key={i} index={i} columns={columnNames} clause={clause} onUpdate={updateWhere} onRemove={removeWhere} />
          ))}
      ...
      <div className='flex items-start gap-2'>
        ...
        <div className='flex flex-wrap gap-1'>
          {columnNames.map(col => (
            <GroupByToggle key={col} col={col} isSelected={state.groupBy.includes(col)} groupBy={state.groupBy} onUpdate={updateGroupBy} />
          ))}
        </div>
      </div>
      ...
      <div className='flex items-start gap-2'>
        ...
        <div className='flex flex-col gap-1.5'>
          {state.orderBy.map((clause, i) => (
            <OrderByRow key={i} index={i} clause={clause} columnNames={columnNames} onUpdate={updateOrderBy} onRemove={removeOrderBy} />
          ))}
