...
const ParamEditor = ({ param, onChange }: { param: FunctionParam; onChange: (param: FunctionParam) => void }) => {
  ...
  if (param.kind === 'range') {
    return <Input value={param.value} onChange={handleRangeChange} placeholder='5m' className='h-6 w-16 text-xs' aria-label='Range duration' />;
  }

  if (param.kind === 'scalar') {
    return <Input value={param.value} onChange={handleScalarChange} placeholder='0' className='h-6 w-16 text-xs' aria-label='Scalar value' />;
  }
  ...
  return (
    <div className='bg-muted/50 flex items-center gap-1.5 rounded-md px-2 py-1'>
      ...
      {fn.params.map((param, pi) => (
        <IndexedParamEditor key={pi} paramIndex={pi} param={param} onParamChange={handleParamChange} />
      ))}
      {entry !== undefined && <span className='text-muted-foreground ml-auto text-[10px]'>{entry.description}</span>}
      <div className='flex items-center gap-0.5'>
        <Button variant='ghost' size='icon-xs' onClick={handleMoveUp} disabled={isFirst} aria-label='Move up'>
          <ArrowUpIcon />
        </Button>
        <Button variant='ghost' size='icon-xs' onClick={handleMoveDown} disabled={isLast} aria-label='Move down'>
          <ArrowDownIcon />
        </Button>
...
export const PromqlFunctionComposer = ({ functions: fns, onAdd, onRemove, onReorder, onParamChange }: PromqlFunctionComposerProps) => {
