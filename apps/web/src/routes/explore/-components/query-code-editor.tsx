...
export const QueryCodeEditor = ({ datasourceType, dialect, schema, promqlClient, value, onChange, onRun, placeholder }: QueryCodeEditorProps) => {
  ...
  useEffect(() => {
    ...
    const extensions = [theme, runKeymap, updateListener, autocompletion(), langCompartment.current.of([]), EditorView.lineWrapping];
  ...
  useEffect(() => {
    ...
    const langExtension = datasourceType === 'sql' ? buildSqlExtension(dialect, schema) : buildPromqlExtension(promqlClient);
